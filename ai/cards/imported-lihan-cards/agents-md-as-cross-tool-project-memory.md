---
id: lihan-cards-agents-md-as-cross-tool-project-memory
kind: card
domain: principle
visibility: team
status: valid
title: Use AGENTS.md + one-line CLAUDE.md @-import as cross-tool project memory
summary: When a project needs the same rules across multiple AI tools (Codex,
  Cursor, Copilot, Claude Code), make AGENTS.md the single source of truth and
  replace CLAUDE.md with one line `@AGENTS.md` — same rules everywhere, no
...
routes:
  ...
  - claude-code
verify:
  command: From a fresh Claude Code session in a project with CLAUDE.md =
    `@AGENTS.md` and AGENTS.md = full rules, run /memory and confirm AGENTS.md
    content is loaded.
  expected: /memory lists CLAUDE.md and shows AGENTS.md expanded inline at the
    @-import point.
  failure_next: Check the @-import is at column 0 of CLAUDE.md; path resolves
    relative to CLAUDE.md's directory; recursive depth cap is 5.
---

> Imported from lihan3238.github.io ai/cards/agents-md-as-cross-tool-project-memory.md. The body is preserved; metadata was converted to AI Workflow Home asset schema.

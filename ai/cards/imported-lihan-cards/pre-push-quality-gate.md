---
id: lihan-cards-pre-push-quality-gate
kind: card
domain: coding
visibility: team
status: valid
title: Pre-push verification gates
summary: Run format / lint / type / test / coverage / change-hygiene locally
  before any git push or PR creation; CI would catch this, but locally is 3x
  faster.
...
routes:
  ...
  - claude-code
verify:
  command: Before pushing, run the project's documented gate suite (per its
    AGENTS.md / CLAUDE.md / README); confirm every gate exits 0; only then run
    git push.
  expected: Local gate set passes; subsequent CI is a re-run of the same checks,
    not a new failure surface.
  failure_next: If a local gate fails, fix and re-run before push. If CI fails
    after local passes, the gate set is incomplete — add the missing check to
    the local suite first.
---

> Imported from lihan3238.github.io ai/cards/pre-push-quality-gate.md. The body is preserved; metadata was converted to AI Workflow Home asset schema.
